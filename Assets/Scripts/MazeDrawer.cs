...
public class MazeDrawer : MonoBehaviour {
    ...
    void Start()
    {
        ...
        for (int i = 0; i < maze.Tiles.GetLength(0); i++)
            ...
            { 
                if (maze.Tiles[i, j].type == Tile.Type.Wall && maze.Tiles[i, j].biome != null)
                {
                    GameObject cube = Instantiate(prefab, new Vector3(TransformToWorldCoordinate(i), 0, TransformToWorldCoordinate(j)), Quaternion.identity);
